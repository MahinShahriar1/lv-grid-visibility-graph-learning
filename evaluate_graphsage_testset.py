...
# model_load_path = f"best_model_static_graphsage6_Phase{PHASE}.pt"
# model_load_path = f"best_model_static_graphsage6_Phase{PHASE}.pt"
model_load_path = f"best_model_static_graphsage_testpyG_Phase{PHASE}.pt"
# =================================================================
# --- 4. DATA LOADING ---
# =================================================================
...
def save_node_timeseries_for_plotting(
    save_dir: str,
    phase: str,
    node_id: int,
    d_interval: int,
    actual_pu: np.ndarray,
    pred_raw_pu: np.ndarray,
    pred_final_pu: np.ndarray,
):
    ...
    interval_idx = np.arange(0, T, int(d_interval), dtype=int)
    is_interval = np.zeros(T, dtype=bool)
    is_interval[interval_idx] = True
    ...
    df = pd.DataFrame({
        ...
        "actual_pu": actual_pu,
        "pred_raw_pu": pred_raw_pu,
        "pred_final_pu": pred_final_pu,
    })

    # errors (easy for later MSE/MAE plotting)
    df["err_raw_pu"] = df["pred_raw_pu"] - df["actual_pu"]
    df["abs_err_raw_pu"] = np.abs(df["err_raw_pu"])
    df["sq_err_raw_pu2"] = df["err_raw_pu"] ** 2

    df["err_final_pu"] = df["pred_final_pu"] - df["actual_pu"]
    df["abs_err_final_pu"] = np.abs(df["err_final_pu"])
    df["sq_err_final_pu2"] = df["err_final_pu"] ** 2

    base = f"node_{node_id}_phase_{phase}"
    full_path = os.path.join(save_dir, base + "_timeseries_full_actual.csv")
    int_path  = os.path.join(save_dir, base + f"_timeseries_interval_actual{int(d_interval)}.csv")

    df.to_csv(full_path, index=False)
    df[df["is_interval"]].to_csv(int_path, index=False)

    print(f"✅ Saved node time-series (full):     {full_path}")
    print(f"✅ Saved node time-series (interval): {int_path}")
...
if node_plot_index != -1:
    ...
    # =================================================================
# --- 7.1 CALIBRATION REMOVED (RAW ONLY) ---
# =================================================================
    node_predictions_full = node_predictions_full_raw.copy()  # FINAL == RAW (no calibration)
    calibration_was_run = False
    calibrator_node = None
    calibrator_name = "None"

    print("\n[Calibration] Removed/Disabled. Using RAW predictions only.")
    ...
    # =================================================================
    # =================================================================
# --- SAVE NODE TIME-SERIES FOR LATER PLOTTING ---
# =================================================================
    SAVE_DIR = os.path.join("saved_timeseries")
    save_node_timeseries_for_plotting(
        save_dir=SAVE_DIR,
        phase=PHASE,
        node_id=NODE_TO_PLOT_ID,
        d_interval=int(d_interval),
        actual_pu=node_actuals_full,
        pred_raw_pu=node_predictions_full_raw,
        pred_final_pu=node_predictions_full,  # same as raw now
    )
...
if interval_indices_plot_global.size > 0:
    ax.scatter(interval_indices_plot_global, flat_preds_interval_plot,
               color="orange", s=10, label=f"Predict @ {d_interval} min", zorder=2)
    ax.scatter(interval_indices_plot_global, flat_actuals_interval_plot,
               color="green", s=10, label=f"Actual @ {d_interval} min", zorder=2)
...
ax.set_xlabel(r"$\mathbf{(e)}$ Time step (min)")
